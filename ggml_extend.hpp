...
#if 0 // Debug output tiles.
    std::string fileName = "tiles-" + std::to_string(x) + "-" + std::to_string(y) + "-" + std::to_string(overlap) + ".dat";
    std::ofstream outFile(fileName, std::ios::binary);
    if (outFile) {
        for (int iy = 0; iy < height; iy++) {
            for (int ix = 0; ix < width; ix++) {
                for (int k = 0; k < channels; k++) {
                    const float new_value = ggml_tensor_get_f32(input, ix, iy, k);
                    outFile.write(reinterpret_cast<const char*>(&new_value), sizeof(float));
                }
            }
        }
        outFile.close();
        ...
        for (int ix = 0; ix < width; ix++) {
            for (int k = 0; k < channels; k++) {
                ...
                if (overlap > 0) {  // blend colors in overlapped area
                    ...
                    float step = 1.0f;
                    step *= y > 0 && iy < overlap ? ggml_smootherstep_f32(iy / float(overlap)) : 1.0f;
                    step *= x > 0 && ix < overlap ? ggml_smootherstep_f32(ix / float(overlap)) : 1.0f;
                    const float final_value = old_value * (1.0f - step) + new_value * step;
...
__STATIC_INLINE__ void sd_tiling(ggml_tensor* input, ggml_tensor* output, const int scale, const int tile_size, const float tile_overlap_factor, on_tile_process on_processing) {
    ...
    int num_tiles_x = ((input_width  - tile_size + non_tile_overlap - 1) / non_tile_overlap) + 1;
    int num_tiles_y = ((input_height - tile_size + non_tile_overlap - 1) / non_tile_overlap) + 1;
    int num_tiles   = num_tiles_x * num_tiles_y;
    LOG_INFO("processing %i tiles", num_tiles);
    for (int j = 0; j < num_tiles_y; ++j) {
        int y = j == num_tiles_y - 1 ? input_height - tile_size : j * non_tile_overlap;
        for (int i = 0; i < num_tiles_x; ++i) {
            int x = i == num_tiles_x - 1 ? input_width - tile_size : i * non_tile_overlap;
            ...
            float last_time  = (t2 - t1) / 1000.0f;
            pretty_progress((i + 1) * (j + 1), num_tiles, last_time);
        }
    }
    ggml_free(tiles_ctx);
}
